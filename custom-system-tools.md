...
You can import the base tools from three different locations, depending on your preference:
...
- `_message_bus` - The message queue backend (RabbitMQ/Redis)
- `_artifact_storage` - The storage backend (MinIO/S3)
- `_agent` - The current agent instance (for job_id propagation)

**Important**: These are injected by the runtime - you don't pass them manually.
...
Define your overrides in your agent Python file (e.g., `my_agent.py`):
...
import pytest
from laddr import override_system_tool, TaskDelegationTool
from laddr.core.system_tools import get_tool_override, clear_tool_overrides
...
def test_custom_override():
    """Test custom delegation override."""
    ...
    async def test_override(
        ...
        **kwargs
        ...
        return {"custom": True, "agent": agent_name}
    
    # Verify registration
    override_func = get_tool_override("system_delegate_task")
    assert override_func is not None
    assert override_func == test_override
    
    # Test execution
    result = await test_override(
...
```python
# Original signature
async def system_delegate_task(
    agent_name: str,
    task_description: str,
    task: str,
    task_data: dict = None,
...
# Your override should match
@override_system_tool("system_delegate_task")
async def my_override(
    agent_name: str,           # Same parameter
    task_description: str,      # Same parameter
    task: str,                 # Same parameter
    task_data: dict = None,    # Same parameter
    timeout_seconds: int = 300, # Same parameter
    _message_bus=None,         # Runtime injection
    _artifact_storage=None,     # Runtime injection
    _agent=None                # Runtime injection
) -> dict:                     # Same return type
    pass
```
...
async def monitored_delegation(
    ...
    """
    Enhanced task delegation with monitoring, metrics, and logging.
    """
    ...
    logger.info(
        f"[DELEGATE] Agent: {agent_name} | Task: {task_description[:50]}"
    )
    
    try:
        # Use base delegation tool
        delegation_tool = TaskDelegationTool(_message_bus, _artifact_storage, _agent)
        ...
        logger.info(
            f"[SUCCESS] Agent: {agent_name} | Duration: {duration:.2f}s"
        )
        ...
        logger.error(
            f"[FAILED] Agent: {agent_name} | Duration: {duration:.2f}s | Error: {e}"
        )
...
def get_delegation_metrics():
    ...
    return {
        ...
        "average_duration": (
            metrics["total_duration"] / metrics["total_delegations"]
...
# my_agent.py
from laddr import Agent
import custom_tools  # Registers the override

class MyAgent(Agent):
    async def process_task(self, task_data):
        # This will use your monitored_delegation override
        result = await self.delegate_task(
            agent_name="worker",
            task_description="Process data",
            task="process",
            task_data={"items": [1, 2, 3]}
        )
        
        # Check metrics
        metrics = custom_tools.get_delegation_metrics()
...
- [System Tools Source Code](../lib/laddr/src/laddr/core/system_tools.py)
- [Example: Custom Parallel Delegation](../../examples/custom_parallel_delegation.py)
- [Example: Verification Tests](../../examples/verify_system_tool_exports.py)
- [API Reference](../reference/evenage.core.system_tools.md)

## Summary

The base system tool classes provide a powerful way to extend Laddr's delegation and storage capabilities:

1. **Import** base tools from `laddr`, `laddr.core`, or `laddr.core.system_tools`
2. **Use `@override_system_tool`** decorator to register your custom implementation
3. **Accept runtime-injected parameters**: `_message_bus`, `_artifact_storage`, `_agent`
4. **Instantiate base tools** with injected dependencies
5. **Add your custom logic** around base tool calls
6. **Test thoroughly** to ensure correct behavior

This approach allows you to enhance system tools with monitoring, rate limiting, circuit breakers, retries, and more—while reusing Laddr's battle-tested core delegation logic.
